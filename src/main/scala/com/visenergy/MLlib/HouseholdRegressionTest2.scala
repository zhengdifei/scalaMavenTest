...
/**
 * 回归：两种回归的方法调优-调整label，取对数，让值更趋近于正态分布
 */
object HouseholdRegressionTest2 {
...
//    val vdata = hdData3.map( record => LabeledPoint(math.log(record._6),Vectors.dense(extract_feature(record))))
    vdata.cache
...
//    outputStringList ::= "feature:" + first_vdata.features
    ...
    val lrPredictData = vdata.map{ point =>
      (math.exp(point.label) , math.exp(lrModel.predict(point.features)))
    }
     ...
     * 
     * 性能评估(c2)，log(label)
     * Linear Model MSE : NaN
     * Linear Model MAE : NaN
     * Linear Model RMSLE : NaN
     * 
     * 性能评估(c2),只有时间属性
     * Linear Model MSE : 1.1292
     * Linear Model MAE : 0.7907
     * Linear Model RMSLE : 0.4256
     */
     ...
     * 决策树回归
     */
    val dtModel = DecisionTree.trainRegressor(vdata,scala.collection.immutable.Map[Int, Int](),"variance",3,100)
    val dtPredictData = vdata.map{ point =>
      (point.label,dtModel.predict(point.features))
//      (math.exp(point.label),math.exp(dtModel.predict(point.features)))
    }
     ...
     * 
     * 性能评估(c2)，log(label)
     * DecisionTree Model MSE : 0.0689
     * DecisionTree Model MAE : 0.1350
     * DecisionTree Model RMSLE : 0.0712
     * 
     * 性能评估(c2),只有时间属性
     * DecisionTree Model MSE : 1.1927
     * DecisionTree Model MAE : 0.7441
     * DecisionTree Model RMSLE : 0.4221 
     */
