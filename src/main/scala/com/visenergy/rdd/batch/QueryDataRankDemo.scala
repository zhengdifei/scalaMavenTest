...
object QueryDataRankDemo {
  def main(args:Array[String]) {
    if (args.length < 4) {
      println("Please input correct params | Usage: startTime(yyyy-MM-dd HH:mm:ss) endTime(yyyy-MM-dd HH:mm:ss) property topN key1 value1 key2 value2...")
      return
    }
...
//    val endTime = BaseConfig.strToTimeStamp(args(1))
    ...
    es.execute(new Runnable() {
      def run() {
        ...
        try {
          val rankResult = rddUtil.getMsgRank(filteredRdd, property).take(topN)
    ...
    es.execute(new Runnable() {
      def run() {
        ...
        val jedis = redisPool.getResource
        try {
          val rankResult = rddUtil.getMsgRank4TargetKey(filteredRdd, property, BaseConfig.SALES_AMOUNT_PROPERTY).take(topN)
