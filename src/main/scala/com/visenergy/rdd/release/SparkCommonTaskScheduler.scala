...
object SparkCommonTaskScheduler {
  def main(args:Array[String]) {
    val conf = new SparkConf().setAppName("SparkCommonTaskScheduler").setMaster("local[2]")
    ...
    var hbaseData = HBase2Spark.getHBaseRdd(sc, 0L, initTime)
    hbaseData.repartition(sc.defaultParallelism * 3)
    var isFirst = true
    ...
    scheduler.scheduleAtFixedRate(new Runnable() {
      def run() {
        ...
        var periodRdd = HBase2Spark.getHBaseRdd(sc, initTime, now)
        
        initTime = now
        
        if (isFirst || periodRdd.count() > 0) {
          
          isFirst = false
          
          hbaseData = hbaseData.union(periodRdd).coalesce(sc.defaultParallelism * 3, false)
