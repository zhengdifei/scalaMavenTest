...
object EmulatorToSpark {
  ...
  def allPriceFromHbase(sc:SparkContext,config:Configuration):RDD[(ImmutableBytesWritable,Result)] = {
    val scan = new Scan()
    scan.setStartRow(Bytes.toBytes("1035177490"))
    scan.setStopRow(Bytes.toBytes("1035177490"))
   ...
   * midCountNum : 中位数下标
   */
  def getMidNum(sc:SparkContext,rdd:RDD[Double],partNum:Int,midPartNum:Int,midCountNum:Int):(RDD[Double],Int) = {
	  ...
	  if(acc.value > midCountNum){
	      result = getMidNum(sc,rdd,partNum,midPartNum-1,midCountNum)
	  ...
	  //中位数正好落在midCountNum分区中
	     ...
	     if(rdd2Count > 20000 ){
	    	 result = getMidNum(sc,rdd2,partNum,midPartNum,midCountNum-acc.value-1)
	     }
	  //其他异常情况
	  }else{
	    throw new Exception()
    ...
    sensorDs.foreachRDD{ rdd => 
      if(!rdd.isEmpty){
	    try{
		    ...
		    rdd.map(hbaseAdaptor(acc_sum,acc_bytes,currentTime,_))saveAsHadoopDataset(insert2HbaseJobConf(getHbaseConfig,"sensorData"))
		    //用累加器统计消息模拟器发出数据总条数
			//初始化redis
		    RedisDao.makePool()
			...
			val df2 = sqlc.sql("select SID,count(SID) as outlet_count,avg(price) outlet_avg_price,sum(price) as outlet_amount from emulator group by SID order by sum_jg")
			...
			//从hbase里面取price的全量数据
			val priceRddResult = allPriceFromHbase(sc,getHbaseConfig)
			val priceRdd = priceRddResult.flatMap(x => {
		    ...
		    val priceMidPartRdd = getMidNum(sc,priceRdd,partNum,midPartNum,midCountNum.toInt)
